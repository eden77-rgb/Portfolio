...
const NavBar = () => {
    ...
    useEffect(() => {
        setShowBack(location.pathname.startsWith("/project"))
    }, [location.pathname])

    return (
        <>
            <nav className="fixed top-6 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6">
                <div className="flex items-center">
                    {showBack &&
                        <GoBackButton content={langJSON.NavBar.ShowBack[lang]} variant="primary" />
                    }
                </div>
